...
Vi har i denne opgave re-implementeret vores MWI filter fra A2 i form
af en co-processor, som styres af processoren i vores tidligere
system. Vi har fundet et fokus område mht.~hvad vi mente var den mest
optimale performance model for co-processoren.

Co-processoren integreres som et yderligere komponent i platformen, og
kommunikerer med CPU'en gennem bussen.

Denne metode giver et, i forhold til A2, endnu bedre billede af en
realistisk dedikeret hardware implementation af et specifikt
komponent, som har én funktion.
...
(såsom skærm, sensore og højtalere) vil vores co-processor sjældent
...
fylder $0.001mm^2$ eller $1mm^2$ er ikke noget, der vil afgøre én
implementationsmetode over en anden.

Vi vælger at fokusere hovedsageligt på hastighedsmæssig
...
Nedenfor ses det et diagram over co-processoren med deres centrale
komponenter:
...
A2 implementationen, da vi undgår en ekstra \code{load} instruktion
for at hente de gamle værdier ud. Dette er dog på bekostning af et
...
være pladsbesparende, men ville formodentlig ikke give en betydelig
...
de seneste 30 værdier i et register (\emph{ACC}). Denne akkumulerede værdi
...
består af tre output signaler og to input mht.~bussen:
...
Vi har brugt samme bus interface som i hovedprocessoren. Derfor er
det ikke alle signalene som bliver brugt. Fordi co-processoren får sin
data fra \code{S\_cmdout} bruger vi ikke \code{S\_dataout}.

\code{S\_datainrdy} bliver sat i \emph{Finite State Machinen} til
co-processoren afhængig om den staller og har fanget dataen.

Da det er givet at data værdiene som bliver givet er positive, bruger
vi ikke two's-complement (\code{tc()}). Dette gør at vi kan bruge
dobbelt så store dataværdier.
...
iterationer gamle værdi. Denne \code{pointer} wrappes på samme måde
som \code{count}.

\subsection{MWI}
Vores \emph{MWI} komponent får to 32-bits værdier som input; en gammel
og en ny, kaldet \code{old} og \code{curr}, respektivt. Yderligere får
den et flag \code{enable} der indikerer hvorvidt der skal opereres
eller ej.

Ud giver den resultatet af dens beregning, som er nyt 32-bits signal
kaldet \code{result}.

Ydermere behøver vi en akkumulator der gemmer resultaterne i
registre. Denne er naturligvis også på 32-bits og kaldes \code{acc}.

For at lave udregningen starter vi med at lægge addere input værdien,
kaldet \code{curr}, og den nuværende akkumulator værdi, for så at
gemme det i et signal \code{add}. Derefter sætter vi et \code{sub}
signal der trækker \code{old} fra \code{add}, hvis \code{enable} er
højt. Hvis ikke sættes \code{sub} lig med akkumulatorens værdi, som
den så sætter lig med sig selv. Resultatet giver den udregnede
\code{sub} værdi, delt med 30.

Vi bruger en hardcoded division. Altså vi bitshifter efter at have
multipliceret med en faktor som tilsvarer 30. Dette er yderligere
forklaret i A2.

\section{Profilering}
\subsection{Areal}
I A2 har vi lavet en fejl i vores udregning af det totale areal af
CPUen. Vi fandt at der var $14306$ transistore i CPUen, og transistor
størrelsen er angivet som $90nm$. Vi antager nu at hver transistor
optager et kvadratisk areal, og at de dermed er $90nm \cdot 90nm =
8100nm^2$. Dermed har vi et samlet areal for CPUen på

\[ 14306 \cdot 8100nm^2 = 115.9\mu m^2 \]

Vi finder nu arealet af MWI co-processoren og lægger denne til det
samlede areal. Det kan vi gøre fordi vi ikke har fjernet noget fra
CPU'en.
...
\begin{table}[H]
  ...
  \begin{tabular}{|c|c|c|c|c|c|c|c|c|}
    \hline \rowcolor{blue!25}
              & Co-pro     & MWI & Regfile  & Total gates & Trans. pr. enh. & Trans. \\
    \hline \cellcolor{blue!25}
    XNOR      & 5          & 0   & 1984     & 1989        & 8               & 15912  \\
    \hline \cellcolor{blue!25}
    AND       & 2          & 0   & 31       & 33          & 6               & 198    \\
    \hline \cellcolor{blue!25}
    MUX       & 0          & 32  & 1984     & 2016        & 6               & 12096  \\
    \hline \cellcolor{blue!25}
    ADDER     & 0          & 64  & 20       & 84          & 28              & 2352   \\
    \hline \cellcolor{blue!25}
    MULT      & 0          & 57  & 0        & 57          & 12              & 684    \\
    \hline \cellcolor{blue!25}
    Flip-Flop & 66         & 32  & 873      & 971         & 12              & 11652  \\
...
Det samlede areal af co-processoren er altså
\[ 42894 \cdot 8100nm^2 = 347.4\mu m^2 \]

Dette er næsten 3 gange så stort som co-processoren, grundet de mange
registre og MUX komponenter. Det er et resultat af vores fokus på
hastighed og ikke plads.

\subsection{Cycles}
Der bliver brugt $7471$ toggles til at behandle hele datasættet. Hver
MWI værdi udregnes på $31$ cycles.
...
med en rate på $250/s$, og vi bruger $31$ cycles på at udregne en MWI
værdi. Vi har altså at
...
output vises. Eksemplevis ser \code{sfg}'erne i \code{mwi\_test.fdl}
ud som følger:
...
\end{lstlisting}
\caption{\code{mwi\_test.fdl}}
\end{figure}


Den korresponderende \code{sequencer} som løber dem igennem, ser sådan
ud:
...
\section{Konlusion}
Vi har i denne opgave lavet en co-processor som udfører \code{MWI}
udregningen af QRS algoritmen. Det vil sige at vi er gået fra C kode i
A1, til en CPU i Gezel og assembler i A2, til en dedikeret hardware
implementation i A3.

Vi har prioriteret hastighed over energi- og pladsforbrug i
implementationen, men har alligevel oplevet en forbedring på næsten
alle områder, undtagen pladsforbrug som blev øget markant.

Vi har undersøgt forskellige måder at implementere dele af
co-processoren og har valgt iblandt dem ud fra ovenstående fokus.

Vi har analyseret performance i forhold til implementationen i A2, som
var uden den dedikerede co-processor, og fandt at der blev sparet
næsten $50\%$ energi, og køretiden blev nedsat med omkring $20\%$.
...
co-processor køres med kommanden
\code{bin/run assembly/mwi\_copro.asm 1000}.
