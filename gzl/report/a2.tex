...
kontrollsignaler og registre som saa bestemmer hvordan de tilkoblede
...
Vi blir ogsaa noedt til aa implementere et kontrollsignal som
forteller de tilkoblede komponenter at de skal vente i tilfellet
bussen er i gang med en \code{load} eller \code{store} operasjon.

\subsubsection{Program Counter} % Norsk
\emph{Program Telleren} styrer hvilken instruksjon som skal
eksikveres, eller naermere paa hvilken adresse i \emph{Instruction
  Memory} den ligger. Normalt vil den ta den forrige instruksjons
adresse og legge én til, altsaa neste linje i programmet. Men, hvis
det forrige instruksjon saa var en jump operasjon skal den neste
instruksjon komme fra den adresse jump instruksjonen ba den om aa
jumpe til. 

\subsubsection{Jump Handler} % Norsk
\emph{Jump Handleren} bestemmer utifra hvilken jump kontrollsignaler
som er satt i \emph{Controller} og \emph{ALU}, om den skal jumpe. Hvis
feks. instruksjonen sier at programmet skal jumpe hvis forrige
vaerdier som ble sammenlignet er negative, og \emph{ALU} viser at de
var det, setter den et flagg som sier at programmet skal jumpe. Dette
flagg ses saa av \emph{Program Counter}, som beskrevet over.
...
hvilken operasjon den skal utfoere, feks. addisjon, suktraksjon,
multiplikasjon eller divisjon, og dataen der skal utfoere de paa. Den
gir saa resultatet og ytligere setter kontrollsignaler som beskriver
om resultatet var negativt eller null.

\subsubsection{Register File} % Norsk
\emph{Register Filen} holder styr pa dataen til de aatte forskjellige
...
og tilfellet i hvilket register og hvilken data der skal gjemmes.
...
Det er nu kun opp til \emph{Kontrolleren} aa selv bestemme hvilken av disse vaerdier den skal
bruke utifra \code{opcode}'n.

For aa bestemme operasjonen bestemt i \code{opcode}'n har vi valgt aa
gjoere ved aa ha et kontrollsignal per instruksjon i
...
binaere verdi. Instruksjonenes sine respektive kontrollsignal vil saa
...
\begin{description}
  ...
  \item[rw\_data(32)] Bestemmer dataen som skal skrives. Vaerdien
    avhenger av instrusjonen. Er det feks. en \code{mov} instruksjon
...
hver på 3-bits, et kontrollsignal \code{write\_enable}, en register
...
Jump handleren benytter 1 stk. 3-bit OR, 1 stk. 3-bit AND og 1
stk. 2-bit AND gate. Jvf. slides fra forelæsning 9, summerer vi, og
har $5-bit AND + 3-bit OR$.

Til AND og OR gates bruges 1 transistor per bit, og vi har altså i alt
$8$ transistore i vores jump handler. Med den givne
transistorstørrelse på $90nm$ er arealet af vores jump handler
...
\begin{table}[H]
  ...
  \begin{tabular}{|c|c|c|c|}
    ...
    total      & 1643001  & 1636453   & \hw{6548} \\  \hline
    cycle      & 9414     & 9375      & \hw{39}   \\  \hline
  \end{tabular}
  \caption{Toggle data fra Gezel}
  \label{table:toggles}
\end{table}

\section{Konklusjon} % Norsk
Flere immediate instruktioner

I dette prosjekt har vi arbeid aa lave en inlejret prosessor hvor vi
til aa starte med kunne kjoere vaares \code{MWI} filter funksjon. Vi
har simulert input fra en sensor vha. en forhaands satt data minne som
vi saa har kjoert vaares filter program paa som gjemmer resultatet
tilbake i data minnet slik at den kan videre analysert.

Vi har valgt aa implementere en \code{assembler} fordi vi syntes det
ga et meget mer effektivt workflow, med at man kan skrive assembly
kode ogsaa teste det med samme, uten aa foerst bruke tid paa aa
oversett the manuelt. I tillegg at man saa kan debugge i assembly kode
og ikke i maskinkode, noe der tar meget tid.
