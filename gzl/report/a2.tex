...
\subsection{Tests} % Norsk
Ved aa bruke pre-processoren har vi derfor kunne ta testene ut ifra en
hver komponents \code{.fdl} fil, og inn i en fil med sammenavnet, men
appended med \code{_test}. I disse test filene har vi saa bare include
den orginale \code{fdl}-fil. Dette gjoer at vi kan trenger aa ta
valget mellom ha test koden liggenede sammen med implementasjonen
eller aa fjerne den fullstendig.
...
på $90\ nm$. Til multiplier operationen har vi brugt et skema over gates\footnote{\url{http://en.wikipedia.org/wiki/Binary\_multiplier\#Example}}.
...
været i stand til at udvikle et fyldestgørende program og køre det på
Gezel hardwaren. Vi er tilfredse med den måde hardwaren blev designet
på, omend det til tider er svært at overskue alle sammenhængende
komponenter på én gang.
...
Under måling af performance/profiling af systemet fandt vi tilgengæld
nogle resultater vi har svært ved at stole på. At CPU'ens samlede
strømforbrug er $0.4355 nanowatt$ er mildest talt urealistisk. Det er
omtrent det samme som energi outputtet som en enkelt nyre
celle. Hvorfor resultatet blev så småt kan der være flere grunde til,
bl.a. at den givne kapacitans er for lille, eller at der er fejl i
vores udregning af aktiviteten. 

Hvis vi skulle arbejde videre på systemet ville vi implementere flere
\emph{immediate} instruktioner, såsom \code{subi}, \code{muli} og
\code{divi}. Det ville også være bekvemt at have \code{movgt} og
\code{movlt} som ville kopiere en værdi fra et register til et andet,
men kun hvis dets værdi var henholdsvis større eller mindre end
destinationen.
...
Vi har vi faatt god innsikt i hvordan strukturere Gezel kode
vha. \emph{C} pre-processoren, med hensikt aa lave mer oversiktlig og
enkapsulert kode. Vi har utnyttet dette til aa lave separate test
filer som vi kan kjoere for aa forsikre oss at enhver komponent stadig
fungerer efter aa ha gjort endringer.

Vi har sett hvordan programmet bruker ressurser som areal og
stroemforbruk, og analysert om hvordan disse vaerdier kunne forbedres,
og hvilken effekt det saa kunne ha paa programmet. Ydermere har vi
gjort flere overvejelser av valg av komponenter, som innebygede Gezel
funksjoner i motsetning til komponenter man skal selv
implementere. Resultatet er et godt strukturert og forholdsvis
oversiktlig system som kjoerer feilfritt i foelge den givne fasit. Med
det set vi oss godt tilfredse.
