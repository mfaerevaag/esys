...
\code{MUX} komponenter, skulle vi have lavet så mange
forskellige. Altså med forskellige bit bredder og antal inputs. Kode
størrelsen ville derfor blevet større, samtidig at vi mener koden ikke
nødvendigvis var blevet mere overskuelig.
...
appended med \code{\_test}. I disse test filer har vi så inkluderet
den orginale \code{fdl}-fil. Dette gør at vi kan trænger at tage
valget mellem at have test koden liggenede sammen med implementionen
eller at fjerne den fuldstændig.
...
Vi har vi fåt god indsigt i hvordan man strukturerer Gezel kode, og
hardware design kode generelt, vha. \emph{C} pre-processoren, med
hensigt på at lave mere overskueligt og enkapsuleret kode. Vi har
udnyttet dette til at lave separate test filer som vi kan køre for at
...
strømforbrug, og analyseret hvordan disse værdier kunne forbedres, og
hvilken effekt det så kunne have på programmet. Ydermere har vi gjort
flere overvejelser omkring valg af komponenter, som indbyggede Gezel
funktioner i modsætning til komponenter man skal selv
implementere. Resultatet er et godt struktureret og forholdsvist
overskueligt system som kører ifølge den givne facit. Med det ser vi
os godt tilfredse.
...
ikke kan tilgå \code{sig}, men kun \code{reg}, og i de fleste tilfælde
er det uhensigtsmæssigt at skulle vente på at et registers output
opdateres.
